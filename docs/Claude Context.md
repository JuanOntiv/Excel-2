...
13. **All entity data flows through an in-memory Zustand store layer (`src/store/`). Do not add a direct API call in a component.** Added 2026-07-30 to stop the same data being refetched by every view (entering Income used to cost 3 requests, switching period 2 more, switching to Expenses 3 more — all identical data).
    - Five stores: `transactionsStore`, `categoriesStore`, `walletsStore`, `goalsStore`, `recurringStore` (the last added 2026-08-03). Each exposes `bootstrap()` (idempotent — safe to call on every mount), `refresh()`, `reset()`, and CRUD methods that mutate the in-memory copy instead of refetching.
    - `AppShell` calls all five `bootstrap()`s on mount; `AuthContext.logout` calls all five `reset()`s. **Adding a sixth store means touching both places** — miss the reset and the next user in the same browser session sees the previous user's data.
...
14. **The in-memory cache goes stale on server-side cascades, and it fails silently. Three known cascades — look for more before adding a mutation.**
    ...
    - **Recurring execution → transactions (→ goals).** `POST /recurring-transactions/{id}/execute` and `POST /execute-pending` both run `_execute_one`, which inserts a real `Transaction`, runs `assign_wallets_for_transaction`, and advances `next_execution`; `execute-pending` additionally runs `evaluate_goal_completions` + `flag_expired_goals`. `recurringStore.executeNow` therefore refreshes transactions and revalidates goals. `AuthContext.login` does the same after `executePending()` resolves, but **only when `executed > 0`**, and it `await`s `bootstrap()` before `refresh()`: transactions load in batches, and two concurrent loads can interleave their `set`s and duplicate rows. This was a real (unreported) bug for months — confirming a pending recurrence left the new movement invisible until a page reload.
...
- `GET /recurring-transactions/` had the same missing-`ORDER BY` problem as `GET /transactions` (and a default `limit=100`), which mattered once `recurringStore` started paging it. Fixed 2026-08-03 with `.order_by(created_at.desc(), id.desc())`; verified with `skip/limit` pages that no row repeats or disappears.
...
- **Login rate limiting was keyed by `request.client.host`, which behind a proxy is the same IP for everyone** — so 5 failed attempts by one person locked out *every* user. Surfaced only after deploying (in local dev each client has a distinct IP). Fixed 2026-08-02: the key is now the normalized submitted email (`_login_key()` in `routes/auth.py`), and `RateLimiter` was rewritten from a sliding window to **progressive backoff** (1 min → 5 min → 15 min, capped; strikes forgotten after 1h without failures; env: `LOGIN_MAX_ATTEMPTS`, `LOGIN_LOCKOUT_LADDER_SECONDS`, `LOGIN_STRIKE_DECAY_SECONDS`). Two things that must not be "simplified" away: failures on **non-existent** emails still count (otherwise 401-vs-429 leaks which emails are registered), and the limiter key is normalized (`strip().lower()`) so case variations can't dodge a lockout. The accepted trade-off is that someone who knows your email can lock you out temporarily. No IP-based second layer was added on purpose — it would need real client IPs first (`--proxy-headers` + `--forwarded-allow-ips` in `docker/backend/Dockerfile`, which is not configured).
- **A session could be killed by a network blip, and a revoked account could keep one alive — opposite failures, both fixed 2026-08-02.** Four separate holes, all in the same flow:
  - `api/client.tsx` treated **any** refresh failure as "session expired" and wiped the tokens. The backend runs on Render's free tier, which sleeps the service, so a cold start (timeout/5xx) routinely logged people out mid-session. Now only a 401/403 *from `/auth/refresh`* — or having no refresh token at all — counts as definitive; transient failures propagate as normal errors and the next 401 retries.
  - `POST /auth/refresh` only validated the token row, never `user.is_active`. A deactivated user kept minting access tokens for up to 30 days; since `get_current_user` *does* reject inactive users, the frontend landed in a limbo (401 on everything → refresh OK → 401 again → rejected silently, never redirected). The refresh now loads the user, and on inactive revokes the whole token family and 401s with the **same generic detail** (distinguishing it would leak that the account exists but is suspended). The interceptor also force-logs-out on a second 401 after a successful refresh — safe because admin routes answer 403, not 401.
  - Neither deactivation path (`DELETE /users/me`, `DELETE /users/{id}/deactivate`) nor `POST /users/me/change-password` called `revoke_all_user_tokens` — only the admin password reset did. All three do now. Changing your own password therefore kills your *current* session too (deliberate: the use case is "someone got into my account"), so `Settings.tsx` logs out and redirects to `/login?session=password_changed` instead of leaving a session that starts failing 15 minutes later.
  - Refresh tokens rotate, so two tabs refreshing at once sent the same token: one won, the other got a 401 on an already-revoked token and died. `client.tsx` now serializes refreshes across tabs with `navigator.locks`; a tab that finds the stored refresh token changed while it waited reuses the access token the other tab left instead of rotating again.
- **Session lifetime is 30 days and there is no idle timeout** — the login page used to claim "expiró por inactividad", which was simply false. Text now says the session is no longer valid. If real idle expiry is ever wanted, it has to be built; don't assume it exists.

## Deployment

- **Frontend on Vercel, backend on Render.** The frontend is a client-routed SPA, so Vercel needs `frontend/vercel.json` rewriting every path to `/index.html` — the Vite preset does **not** add that fallback on its own. Without it every deep link 404s (`/login`, `/dashboard`), which also broke the session-expiry redirect, since `handleSessionExpired()` navigates hard rather than through React Router. The file must live in whatever directory Vercel has set as **Root Directory**.
- CORS origins are no longer hardcoded in `main.py`: `CORS_ALLOWED_ORIGINS` (comma-separated) plus `CORS_ALLOWED_ORIGIN_REGEX` in `core/config.py`, so Render env vars can change them without a code edit. The regex covers Vercel preview deploys, which get a fresh subdomain per branch/commit — it is anchored to the project name on purpose; `.*\.vercel\.app` would open the API to anything hosted on Vercel.
...
- `GET /recurring-transactions/pending-confirmation/list` — superseded by `pendingConfirmation()` in `store/selectors.ts` (2026-08-03). Same predicate as the route (`is_active && status=active && !auto_execute && next_execution <= today`), verified against the live endpoint on a mixed set (paused / cancelled / auto_execute / future). One deliberate difference: "today" is the **browser's** local date, not the server's `date.today()`, so on the day boundary across time zones the banner can be off by a day. Accepted — it only decides what is *shown*; the server still decides what actually executes.
...
│   ├── recurringStore.ts     # recurrencias; carga en lotes de 500 (sin estado "partial"), executeNow() invalida transacciones+metas
│   └── selectors.ts          # pure filters over the arrays: byType, byWallet (uses wallet_ids!), byPeriod, summarize,
│                               pendingConfirmation (recurrentes que esperan confirmación — reemplaza un endpoint)
...
The store layer covers **five** resources: transactions, categories, wallets, goals, recurring. For those, a component importing from `api/` directly is a bug — it bypasses the cache and reintroduces the duplicate-fetching the stores exist to kill. Everything else still calls `api/` directly and that's correct, because it has no store: `walletRules`, `logs`, `admin`, `users`, `auth`. Two deliberate exceptions inside the covered five: `exportTransactions`, which streams a file to disk rather than holding state (direct call from `TransactionsView` and `WalletDetail`), and `executePending`, which is a login-time side effect rather than a read (called from `AuthContext`, which then syncs the stores itself).
